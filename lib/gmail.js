...
// Shared booking-extraction pipeline used by both the OAuth Gmail API path
// and the IMAP login path: promo filter, known templates, optional Claude
// extraction, generic fallback. Returns the parsed booking, or null when the
// email isn't a booking.
export async function parseBookingEmail({ text, plaintext, subject, sender }, settings) {
...
export async function parseGmailApiMessage(message, settings) {
  ...
  const parsed = await parseBookingEmail({ text, plaintext, subject, sender }, settings);
