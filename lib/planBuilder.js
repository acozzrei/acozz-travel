...
function optionPhoto(p) {
  ...
  if (p.photoRef) params.set("ref", p.photoRef);
  // Always send coordinates too so the proxy can fall back to Street View
  // when Google has no place photo for this venue.
  if (p.lat != null && p.lng != null) {
    params.set("lat", String(p.lat));
    params.set("lng", String(p.lng));
  }
  const qs = params.toString();
  if (!qs) return { photoUrl: null, photoSource: null };
  return {
    photoUrl: `/api/places/photo?${qs}`,
    photoSource: p.photoRef ? "places" : "streetview",
  };
}
